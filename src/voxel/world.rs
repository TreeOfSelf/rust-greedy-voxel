...
pub fn change_block(world: &mut World, x: i32, y: i32, z: i32, block: u8) {
	...
	let block_position: [i32; 3] = [x - chunk_position[0]*CHUNK_SIZE as i32, y - chunk_position[1]*CHUNK_SIZE as i32, z - chunk_position[2]*CHUNK_SIZE as i32];
	let block_id: i32 = block_position[0]+block_position[1]*CHUNK_SIZE as i32+block_position[2]*CHUNK_SIZE as i32*CHUNK_SIZE as i32;
    ...
    match &chunk.volume {
        Some(shared_array_buffer) => {
            ...
            volume_array.set_index(block_id as u32,block);
        }        
        None => {}
    }
    mesh::greedy(chunk);
...
pub fn consume_chunk_buffer(world: &mut World, chunk_index: i32, 
    ...
    chunk.block_type = Some(block_type_buffer);
    chunk.texture_coordinates = Some(texture_coordinates_buffer);
}